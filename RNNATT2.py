...
def key_words(new_reviews_ints, alphas):
    
    alphas=alphas.flatten()
    alphas.shape = (len(new_reviews_ints),seq_len)
    
    index = [np.argmax(a[seq_len-len(n):]) for a,n in zip(alphas,new_reviews_ints)]
    print(index)
    int_words = [sen[i] for i,sen in zip(index,new_reviews_ints)]
    print(int_words)
    key_words = [[key for key, value in vocab_to_int.items() if value == int_word][0] for int_word in int_words]
    
    return key_words

#def key(line_int,alphas):
#    
#    alphas = alphas.flatten()
#    print(alphas)
#    index=np.argmax(alphas[seq_len-len(line_int):])
#    
#    int_words = line_int[index]
#    key_words = [key for key, value in vocab_to_int.items() if value == int_words][0]
#    
#    return key_words
...
if __name__ == '__main__':
    ...
    if (args.pos_data and args.neg_data):
    ...
    if (args.new_reviews):        
        ...
        for sen in new_reviews:
            new_reviews_ints.append([vocab_to_int[word] if word in vocab_to_int else vocab_to_int['<UNK>'] for word in sen])
                
        seq_len = 80
    
        labels = np.array([0]*len(reviews_neg))
        labels = labels.reshape([-1,1])
        labels = np.hstack((1-labels, labels))        
        
        new_reviews_ints,_ = clean_reviews(new_reviews_ints,labels)
        new_x = padding(new_reviews_ints,seq_len)
        print(new_x[20:25])
...
#        labels_ = tf.placeholder(tf.int32, [None, num_classes], name="labels")
        ...
        with tf.Session() as sess:
            ...
            for batch_x, batch_y in get_batches(new_x, labels, batch_size=64):
#            for line_int in new_reviews_ints:
...
#            saver.restore(sess, tf.train.latest_checkpoint('checkpoints'))
#            
##            for line_int, batch_y in get_batches(train_x, train_y, batch_size):
#            for line_int in new_reviews_ints:
#                
#                new_x = padding([line_int],seq_len)
#                print(new_x)
#                feed = {inputs_: new_x,
#                            labels_: [[0,1]],
#                            keep_prob: 1}
